...
use crate::models::{Admin, PaginatedResponse, PaginationParams, User};
use axum::{
...
pub async fn create_course(
    ...
    let course = Course {
        ...
        estimated_duration: payload
            .estimated_duration
            .unwrap_or_else(|| "4 weeks".to_string()),
...
pub async fn update_course(
    ...
    if let Some(v) = payload.title_id {
        update.insert("title_id", v);
    }
    if let Some(v) = payload.description {
        update.insert("description", v);
    }
    if let Some(v) = payload.description_id {
        update.insert("description_id", v);
    }
    if let Some(v) = payload.cover_image_url {
        update.insert("cover_image_url", v);
    }
    if let Some(v) = payload.is_published {
        update.insert("is_published", v);
    }
    if let Some(v) = payload.order {
        update.insert("order", v);
    }
    if let Some(ref v) = payload.status {
        update.insert("status", mongodb::bson::to_bson(v).unwrap());
    }
    if let Some(ref v) = payload.skill_focus {
        update.insert("skill_focus", mongodb::bson::to_bson(v).unwrap());
    }
    if let Some(ref v) = payload.target_age {
        update.insert("target_age", mongodb::bson::to_bson(v).unwrap());
    }
    if let Some(ref v) = payload.estimated_duration {
        update.insert("estimated_duration", v);
    }
    if let Some(v) = payload.is_paid {
        update.insert("is_paid", v);
    }
    if let Some(v) = payload.enrollment_cap {
        update.insert("enrollment_cap", v);
    }
    if let Some(ref v) = payload.visibility {
        update.insert("visibility", mongodb::bson::to_bson(v).unwrap());
    }
...
pub async fn create_module(
    ...
    let course_id = ObjectId::parse_str(&payload.course_id)
        .map_err(|_| AppError::BadRequest("Invalid course_id".to_string()))?;
    let collection: Collection<Module> = state.db.database("rustapi").collection("modules");

    let prerequisite_oid = payload
        .prerequisite_id
...
pub async fn list_modules(
    ...
    if let Some(cid) = &params.course_id {
        let oid =
            ObjectId::parse_str(cid).map_err(|_| AppError::NotFound("Not found".to_string()))?;
...
pub async fn update_module(
    ...
    if let Some(v) = payload.is_optional {
        update.insert("is_optional", v);
    }
    if let Some(v) = payload.passing_score_threshold {
        update.insert("passing_score_threshold", v);
    }
    if let Some(ref v) = payload.skill_tags {
        update.insert("skill_tags", mongodb::bson::to_bson(v).unwrap());
    }
    if let Some(ref v) = payload.tags {
        update.insert("tags", mongodb::bson::to_bson(v).unwrap());
    }
...
pub async fn create_lesson(
    ...
    let module_oid = ObjectId::parse_str(&payload.module_id)
        .map_err(|_| AppError::BadRequest("Invalid module_id".to_string()))?;
...
pub async fn list_lessons(
    ...
    if let Some(mid) = &params.module_id {
        let oid =
            ObjectId::parse_str(mid).map_err(|_| AppError::NotFound("Not found".to_string()))?;
...
pub async fn update_lesson(
    ...
    if let Some(v) = payload.content {
        update.insert("content", v);
    }
    if let Some(v) = payload.content_id {
        update.insert("content_id", v);
    }
    if let Some(v) = payload.instruction {
        update.insert("instruction", v);
    }
    if let Some(v) = payload.instruction_id {
        update.insert("instruction_id", v);
    }
    if let Some(v) = payload.culture_notes {
        update.insert("culture_notes", v);
    }
...
pub async fn create_vocabulary(
    ...
    let lesson_id = ObjectId::parse_str(&payload.lesson_id)
...
pub async fn update_vocabulary(
    ...
    let mut update = doc! {};
    if let Some(v) = payload.word {
        update.insert("word", v);
    }
    if let Some(v) = payload.translation {
        update.insert("translation", v);
    }
    if let Some(v) = payload.pronunciation {
        update.insert("pronunciation", v);
    }
    if let Some(v) = payload.audio_url {
        update.insert("audio_url", v);
    }
    if let Some(v) = payload.example_en {
        update.insert("example_en", v);
    }
    if let Some(v) = payload.example_id {
        update.insert("example_id", v);
    }
    ...
    let updated = collection
        ...
        .ok_or(AppError::NotFound("Not found".to_string()))?;
    Ok(Json(
        serde_json::to_value(updated).map_err(|_| AppError::InternalServerError)?,
    ))
}
...
pub async fn update_dialogue(
    ...
    if let Some(v) = payload.branching_tree {
        let bson_tree = mongodb::bson::to_bson(&v).map_err(|_| AppError::InternalServerError)?;
        update.insert("branching_tree", bson_tree);
    }
...
pub async fn update_quiz(
    ...
    if let Some(v) = payload.passing_score {
        update.insert("passing_score", v);
    }
    if let Some(v) = payload.xp_reward {
        update.insert("xp_reward", v);
    }
...
pub async fn public_list_courses(
    ...
    // Section 16: Data-driven visibility — only status=published AND is_published=true
    let mut filter = doc! { "is_published": true, "status": "published" };
    ...
    let cursor = collection.find(filter).sort(doc! { "order": 1 }).await?;
    let data: Vec<Course> = cursor.try_collect().await?;
...
pub async fn public_list_modules(
    ...
    let oid =
        ObjectId::parse_str(&course_id).map_err(|_| AppError::NotFound("Not found".to_string()))?;
    let collection: Collection<Module> = state.db.database("rustapi").collection("modules");

    let cursor = collection
        .find(doc! { "course_id": oid, "is_published": true })
        .sort(doc! { "order": 1 })
        .await?;
    let data: Vec<Module> = cursor.try_collect().await?;
...
pub async fn public_list_lessons(
    ...
    let oid =
        ObjectId::parse_str(&module_id).map_err(|_| AppError::NotFound("Not found".to_string()))?;
    let collection: Collection<Lesson> = state.db.database("rustapi").collection("lessons");

    let cursor = collection
        .find(doc! { "module_id": oid, "is_published": true })
        .sort(doc! { "order": 1 })
        .await?;
    let data: Vec<Lesson> = cursor.try_collect().await?;
...
pub async fn public_get_lesson(
    ...
    let vocab: Vec<Vocabulary> = state
        .db
        .database("rustapi")
        .collection::<Vocabulary>("vocabulary")
        .find(doc! { "lesson_id": oid })
        .await?
        .try_collect()
        .await?;

    let dialogues: Vec<Dialogue> = state
        .db
        .database("rustapi")
        .collection::<Dialogue>("dialogues")
        .find(doc! { "lesson_id": oid })
        .await?
        .try_collect()
        .await?;
...
pub async fn public_recommendations(
    ...
    let scenario_collection: Collection<InterviewScenario> = state
        .db
        .database("rustapi")
        .collection("interview_scenarios");
    ...
    let course_cursor = course_collection
        .find(doc! {
            "is_published": true,
    ...
    if exact_courses.is_empty() {
        let fallback_cursor = course_collection
            .find(doc! {
                "is_published": true,
                "level": { "$regex": format!("^{}$", user_level), "$options": "i" },
            })
    ...
    let scenario_cursor = scenario_collection
        .find(doc! {
            "is_active": true,
    ...
    if exact_scenarios.is_empty() {
        let fallback_cursor = scenario_collection
            .find(doc! {
                "is_active": true,
                "difficulty": { "$regex": format!("^{}$", user_level), "$options": "i" },
            })
...
pub async fn activate_api_key(
    ...
    col.update_many(doc! {}, doc! { "$set": { "is_active": false } })
        .await?;
    // Activate the selected one
    col.update_one(doc! { "_id": oid }, doc! { "$set": { "is_active": true } })
        .await?;
...
async fn get_active_key(state: &Arc<AppState>) -> Result<LlmApiKey, AppError> {
    let col: Collection<LlmApiKey> = state.db.database("rustapi").collection("llm_api_keys");
    col.find_one(doc! { "is_active": true })
        .await?
        .ok_or(AppError::NotFound("Not found".to_string()))
}
    ...
    if !res.status().is_success() {
        ...
        if status == axum::http::StatusCode::TOO_MANY_REQUESTS {
            ...
            let msg = error_json["error"]["message"]
                .as_str()
                .unwrap_or("Gemini API quota exceeded.");
...
pub async fn translate_text(
    ...
    let to_lang = payload
        .to
        .as_deref()
        .unwrap_or("Indonesian (Bahasa Indonesia)");
    ...
    let parsed: serde_json::Value =
        serde_json::from_str(&result).unwrap_or(serde_json::json!({ "translated": result }));
    ...
    let default_prompt = match payload.entity_type.as_str() {
        "course" => format!(
            "Generate a JSON object for a new English learning course for the {}. \
             ...
             Make it professional and engaging for Indonesian learners.",
            context
        ),
        "module" => format!(
        ...
        "lesson" => format!(
            "Generate a JSON object for a new lesson within a {} English course. \
             ...
             Make it professional, practical, and engaging for Indonesian learners.",
            context
        ),
        "game" => format!(
            "Generate a JSON object for a hospitality English mini-game. Context: {}. \
             ...
             Generate relevant content. Content must be relevant to hospitality industry.",
            context
        ),
        _ => return Err(AppError::InternalServerError),
    };

    let prompt_col: Collection<AIPromptConfig> =
        state.db.database("rustapi").collection("ai_prompts");
    let custom_prompt = prompt_col
        .find_one(doc! { "entity_type": payload.entity_type.clone() })
        .await?;
...
async fn save_content_version(
    ...
    let col: Collection<ContentVersion> =
        state.db.database("rustapi").collection("content_versions");

    let opts = FindOneOptions::builder()
        .sort(doc! { "version": -1 })
        .build();
    let latest = col
        .find_one(doc! { "entity_id": entity_id, "entity_type": entity_type })
        .with_options(opts)
        .await?;
...
pub async fn list_content_versions(
    ...
    let oid =
        ObjectId::parse_str(&entity_id).map_err(|_| AppError::NotFound("Not found".to_string()))?;
    let col: Collection<ContentVersion> =
        state.db.database("rustapi").collection("content_versions");

    let opts = FindOptions::builder().sort(doc! { "version": -1 }).build();
    let cursor = col
        .find(doc! { "entity_type": entity_type, "entity_id": oid })
        .with_options(opts)
        .await?;
...
pub async fn rollback_content_version(
    ...
    let cv = col
        .find_one(doc! { "entity_type": &entity_type, "entity_id": oid, "version": version })
        .await?
        .ok_or(AppError::NotFound("Not found".to_string()))?;
        ...
        "course" => {
            ...
            if let Ok(Some(curr)) = coll.find_one(doc! { "_id": oid }).await {
                let _ = save_content_version(
                    ...
                    oid,
                    admin.email.clone(),
                    mongodb::bson::to_document(&curr).unwrap(),
                    Some(format!("Auto-saved before rollback to v{}", version)),
                )
                .await;
            }
            let mut snap = cv.snapshot.clone();
            snap.remove("_id");
            coll.update_one(doc! { "_id": oid }, doc! { "$set": snap })
                .await?;
        }
        "module" => {
    ...
    Ok(Json(
        serde_json::json!({ "message": "Rollback successful" }),
    ))
}
...
pub async fn update_ai_prompt(
    ...
    let opts = mongodb::options::UpdateOptions::builder()
        .upsert(true)
        .build();
    col.update_one(doc! { "entity_type": &entity_type }, update_doc)
        .with_options(opts)
        .await?;

    let updated = col
        .find_one(doc! { "entity_type": &entity_type })
        .await?
        .unwrap();
    Ok(Json(updated))
}
