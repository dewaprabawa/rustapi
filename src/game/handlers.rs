...
use crate::models::{Admin, PaginatedResponse, User};
use axum::{
...
pub async fn create_game(
    ...
    let ai_scenario_id = match payload.ai_scenario_id.as_deref() {
        Some(ai_id) if !ai_id.trim().is_empty() => Some(
            ObjectId::parse_str(ai_id).map_err(|_| AppError::NotFound("Not found".to_string()))?,
        ),
        _ => None,
    };

    let module_id = match payload.module_id.as_deref() {
        Some(mid) if !mid.trim().is_empty() => Some(
...
pub async fn update_game(
    ...
    if let Some(v) = payload.instructions {
        update.insert("instructions", v);
    }
    if let Some(v) = payload.difficulty {
        update.insert("difficulty", v);
    }
    if let Some(v) = payload.asset_url {
        update.insert("asset_url", v);
    }
    if let Some(v) = payload.data_json {
        update.insert(
            "data_json",
            mongodb::bson::to_document(&v).unwrap_or(doc! {}),
        );
    }
    if let Some(v) = payload.ai_scenario_id {
        if !v.trim().is_empty() {
            let ai_id =
                ObjectId::parse_str(&v).map_err(|_| AppError::NotFound("Not found".to_string()))?;
...
pub async fn public_list_lesson_games(
    ...
    let l_id =
        ObjectId::parse_str(&lesson_id).map_err(|_| AppError::NotFound("Not found".to_string()))?;
    let collection: Collection<GameContent> = state.db.database("rustapi").collection("games");

    let cursor = collection
        .find(doc! { "lesson_id": l_id, "is_active": true })
        .sort(doc! { "order": 1 })
        .await?;
...
pub async fn submit_game_result(
    ...
    let game_id = ObjectId::parse_str(&payload.game_id)
        .map_err(|_| AppError::NotFound("Not found".to_string()))?;
    let user_id = user.id.unwrap();

    let collection: Collection<GameResult> =
        state.db.database("rustapi").collection("game_results");
    ...
    Ok(Json(
        serde_json::json!({ "message": "Result saved successfully" }),
    ))
}
...
pub async fn submit_speaking_result(
    ...
    let collection: Collection<SpeakingResult> =
        state.db.database("rustapi").collection("speaking_results");
    ...
    Ok(Json(
        serde_json::json!({ "message": "Speaking result saved successfully" }),
    ))
}
